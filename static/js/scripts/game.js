const hangmanApi = function() {
    ...
    const setGuessButtonsToFalse = () => {
        /* SET THE GUESS BUTTON DATA ATTRIBUTE TO FALSE, 
        USED LATER TO STOP BUTTONS MAKING REQUESTS BEFORE A WORD IS GENERATED */
    ...
    const setGuessButtonsToLetter = () => {
        /* SETS THE DATA ATTRIBUTE TO THE LETTER STORED IN THE VALUE ATTRIBUTE, 
                READY FOR DATA TO BE SENT TO THE SERVER */
        for (let i = 0; i < guessButton.length; i++) {
    ...
    function onLetterClick(e) {
        ...
        let username = document.getElementById("generate").value;
    ...
    generate.addEventListener("click", onGetWordClick); // RUN XHR REQUEST TO RETRIEVE A WORD //

    for (let i = 0; i < guessButton.length; i++){

        guessButton[i].addEventListener("click", onLetterClick); // RUN XHR REQUEST TO SEND USER INPUT DATA //
